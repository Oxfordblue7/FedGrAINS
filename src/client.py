...
class Client_GC():
    ...
    def local_train(self, local_epoch):
        ...
        train_stats = train_gc(self.model, self.name, self.dataLoader, self.optimizer, local_epoch, self.args.device)
...
def train_gc(model, name, dataloaders, optimizer, local_epoch, device):
