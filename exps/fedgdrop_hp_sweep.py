...
def process_fedavg(clients, server, args = None):
    ...
    if args.repeat is None:
        ...
        outfile = os.path.join(args.outpath, "repeats", f'{args.repeat}_accuracy_fedavg_{args.batch_size}_GC{args.suffix}.csv')
...
def main_sweep(config= None):
    ...
    args.outpath = os.path.join(args.outbase, f'{args.dataset}-{args.partition}-{ov}-{args.num_clients}clients-{args.model}-w{gfn}/exp_{args.exp_num}')
    ...
    if args.repeat is not None:
        Path(os.path.join(args.outpath, 'repeats')).mkdir(parents=True, exist_ok=True)
...
if __name__ == '__main__':
    ...
    sweep_config['name'] = f'{args["dataset"]}-{args["partition"]}-{args["num_clients"]}-{ov}-{gfn}-GFlowNet-Sweep'
    # import pprint
